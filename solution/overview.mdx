...
**my Connected Fleet ** allows for the creation of a group hierarchy. You have only one primary group hierarchy, but you can create many subgroups within the hierarchy to meet your reporting/tracking requirements.
...
**Tracking** is knowing the location of a resource in real time. In addition to the location, my Connected Fleet captures speed, direction of travel and other information items depending on the vehicle type (e.g. temperture events).

**Tracking** is knowing the locations where a resource has been. Historic location information is stored by my Connected Fleet based on the customer’s data retention period. This period is configurable,please speak to your **Customer Service Representative** for details.


### 2.4 Journey

A journey is a vehicle moving between locations. The start of the journey is when the ignition is switched on and the end of the journey is when the ignition is switched off. There can be many journeys for a vehicle on the same day. By associating a driver with a vehicle or if the driver has a ID key fob, allows you to track and trace the journeys for a driver.
...
Tags can be used in addition to the **Connect** features of groups and vehicle types. There is no limit on the number of tags you can create.
