...
<Note>
  Drivers cannot be deleted using the API. The \*\*delete \*\*operation will set the driver as inactive.
</Note>
...
The MICHELIN Connected Fleet portal features that a user can access, are dependent on their assigned role. You can define the features available for a role and can create as many roles as needed.
...
Only read operations are available through the API. See the [**Inputs**](/solution/inputs) API for more details.